...
final class DetailCustomLayout: UICollectionViewLayout {
  ...
  enum Element: String {
    ...
    case sectionHeader
    case sectionFooter
    case DetailPageCollectionViewCell
...
extension DetailCustomLayout {
  ...
  override public func prepare() {
    ...
    let headerAttributes = CustomLayoutAttributes(
      forSupplementaryViewOfKind: Element.DetailHeaderView.kind,
      with: IndexPath(item: 0, section: 0)
    )
    prepareElement(size: headerSize, type: .DetailHeaderView, attributes: headerAttributes)
    
    let menuAttributes = CustomLayoutAttributes(
      forSupplementaryViewOfKind: Element.DetailMenuView.kind,
      with: IndexPath(item: 0, section: 0))
    prepareElement(size: menuSize, type: .DetailMenuView, attributes: menuAttributes)
    ...
    for section in 0 ..< collectionView.numberOfSections {
      ...
      for item in 0 ..< collectionView.numberOfItems(inSection: section) {
        ...
        if cellIndexPath.item%2 == 0 {
            ...
            attributes.frame = CGRect(
                x: settings.minimumInteritemSpacing + cellWidth + settings.minimumInteritemSpacing,
              y: contentHeight + lineInterSpace,
              width: itemSize.width,
              height: itemSize.height
            )
        ...
        cache[.DetailPageCollectionViewCell]?[cellIndexPath] = attributes
        zIndex += 1
      }
  ...
  private func prepareCache() {
    cache.removeAll(keepingCapacity: true)
    cache[.DetailHeaderView] = [IndexPath: CustomLayoutAttributes]()
    cache[.DetailMenuView] = [IndexPath: CustomLayoutAttributes]()
    cache[.sectionHeader] = [IndexPath: CustomLayoutAttributes]()
    cache[.sectionFooter] = [IndexPath: CustomLayoutAttributes]()
    cache[.DetailPageCollectionViewCell] = [IndexPath: CustomLayoutAttributes]()
  }
  ...
  private func updateZIndexes(){
    ...
    cache[.DetailMenuView]?.first?.value.zIndex = sectionHeadersZIndex
  }
}
...
extension DetailCustomLayout {
  ...
  public override func layoutAttributesForSupplementaryView(ofKind elementKind: String, at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
    ...
    case Element.DetailHeaderView.kind:
      return cache[.DetailHeaderView]?[indexPath]
      
    default:
      return cache[.DetailMenuView]?[indexPath]
    }
  }
  
  override public func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
    return cache[.DetailPageCollectionViewCell]?[indexPath]
  }
    ...
    for (type, elementInfos) in cache {
      for (indexPath, attributes) in elementInfos {
        ...
        if attributes.frame.intersects(rect) {
          if type == .DetailPageCollectionViewCell,
